...
from utils import realized_volatility_utils as rv_utils
...
class Parkinsons(HistoricalVolatilityStrategy):
    def calculate(self, ticker: str, start: str, end: str = None, realized_volatility_period: str = "M", freq: str = "D") -> list[float]:
        ...
        high_low, dates = process_price_data.get_high_low_rets(ticker, start, end, freq)
        high_low = np.array(high_low) ** 2
        high_low = high_low[::-1]
        
        parkinson = np.array([np.sqrt((1 / (4 * np.log(2))) * sum(high_low[0 + i: length_int + i])) for i in range(len(high_low) - length_int)])[::-1]
        
        return parkinson * np.sqrt(t / length_int), dates[length_int:]
...
class RogerSatchell(HistoricalVolatilityStrategy):
    def calculate(self, ticker: str, start: str, end: str = None, realized_volatility_period: str = "M", freq: str = "D") -> list[float]:
        ...
        high_close, dates = process_price_data.get_high_close_rets(ticker, start, end, freq)
        high_open, _ = process_price_data.get_high_open_rets(ticker, start, end, freq)
        low_close, _ = process_price_data.get_low_close_rets(ticker, start, end, freq)
        low_open, _ = process_price_data.get_low_open_rets(ticker, start, end, freq)

        high_close, high_open, low_close, low_open = np.array(high_close), np.array(high_open), np.array(low_close), np.array(low_open)
        ...
        return np.sqrt(roger_satchell / length_int) * np.sqrt(t), dates[length_int:]
...
class GarmanKlass(HistoricalVolatilityStrategy):
    def calculate(self, ticker: str, start: str, end: str = None, realized_volatility_period: str = "M", freq: str = "D") -> list[float]:
        ...
        high_low, dates = process_price_data.get_high_low_rets(ticker, start, end, freq)
        close_open, _ = process_price_data.get_close_open_rets(ticker, start, end, freq)
        high_low, close_open = np.array(high_low)**2, np.array(close_open) **2
        ...
        return garman_klass * np.sqrt(t), dates[length_int:]
...
class YangZhang(HistoricalVolatilityStrategy):
    def calculate(self, ticker: str, start: str, end: str = None, realized_volatility_period: str = "M", freq: str = "D") -> list[float]:
        ...
        high_low, dates = process_price_data.get_high_low_rets(ticker, start, end, freq)
        close_open, _ = process_price_data.get_close_open_rets(ticker, start, end, freq)
        open_close, _ = process_price_data.get_open_close_rets(ticker, start, end, freq)
        high_low, close_open, open_close = np.array(high_low) ** 2, np.array(close_open) ** 2, np.array(open_close) ** 2
        ...
        return yang_zhang * np.sqrt(t), dates[length_int:]
    
    def __str__(self):
        return "Yang Zhang"
    

class EWMA(HistoricalVolatilityStrategy):
    pass


class GARCH(HistoricalVolatilityStrategy):
    pass


VOLATILITY_REGISTRY = {
    "close_to_close": CloseToClose,
    "parkinson": Parkinsons,
    "rogers_satchell": RogerSatchell,
    "garman_klass": GarmanKlass,
    "yang_zhang": YangZhang,
}


def get_realized_vol_strategy(name: str) -> HistoricalVolatilityStrategy:
    try:
        return VOLATILITY_REGISTRY[name.lower()]()
    except KeyError:
        raise ValueError(f"Unknown realized volatility method: {name}")