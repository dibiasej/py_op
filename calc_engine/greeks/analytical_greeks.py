...
class AnalyticalTheta:
    ...
    def calculate(S: float, K: float, T: float, sigma: float, r: float = 0.0, q: float = 0.0, otype: str = "call") -> float:
        if T == 0:
            T = .5/365  # Assume user passed T in trading days
...
class AnalyticalVanna:
    ...
    def calculate(self, S: int, K: int, T: float, sigma: float, r: float = 0, q: float = 0, otype: str = "call") -> float:
        ...
        #vanna = -np.exp(-q*T)*np.sqrt(T)*norm.pdf(d1)*(d2/sigma)
        vanna = -np.exp(-q*T)*norm.pdf(d1)*(d2/sigma) # this vanna gives the correct gvv implied spot vol cov
        return vanna
...
class AnalyticalVolga:
    ...
    def calculate(self, S: int, K: int, T: float, sigma: float, r: float = 0, q: float = 0, otype: str = "call") -> float:
        ...
        vega: float = S*np.exp(-q*T) * norm.pdf(d1) * np.sqrt(T)
        volga = vega * ((d1*d2) / sigma) # this volga gives the correct vol var parameter
        return volga
...
class AnalyticalZomma:

    def calculate(self, S, K, T, sigma, r=0.0, q=0.0):
        if T > 1:
            T /= 365
...
class AnalyticalUltima:
    ...
    def calculate(self, S, K, T, sigma, r=0.0, q=0.0):
        ...
        # chat gpt ultima
        #ultima = volga * (1/sigma) * (d1*d2 - d1/d2 - d2/d1 - 1)

        # wiki ultima
        ultima = -vega * (1 / sigma**2) * (d1*d2 * (1 - d1*d2) + d1**2 + d2**2)
        return ultima
...
class AnalyticalVolgaSpot:
    """
    I got this from chat gpt I am not sure if it is correct
    """
    ...
    def calculate(self, S, K, T, sigma, r=0.0, q=0.0):
        ...
        term = (d1 * d2 + d1 + d2) / sigma

        dVolga_dS = (vega / S) * term

        scaled = 0.5 * sigma**2 * dVolga_dS
