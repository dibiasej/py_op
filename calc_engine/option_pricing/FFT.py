...
class HestonFFT(FFT_Template):
    ...
    def __init__(self, characteristic_function = cf.heston) -> None:
        self.characteristic_function = characteristic_function

    def call(self, S: float, strikes: list[int], T: float, K: int = None, r: float = .02, v0: float = .04, theta: float = .02, kappa: float = 2, sigma: float = .3, rho: float = -.5, interp = 'cubic'):
        """
    ...
    def put(self, S: float, strikes: list[int], T: float, K: int = None, r: float = .02, v0: float = .04, theta: float = .02, kappa: float = 2, sigma: float = .3, rho: float = -.5, interp = 'cubic'):
        """
        FFT method. It returns a vector of prices.
        K is an array of strikes
        """
