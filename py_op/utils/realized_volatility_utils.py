...
def realized_volatility_period_length(realized_volatility_period: str, data_freq: str) -> int:
    ...
    # FIX: detect leading digit properly
    if len(length) >= 2 and length[0].isdigit():
        length = length[:2]
    else:
        length = length[:1]
    ...
    match length:
        ...
        case "2y":
            return round(504 / scale) if period == "D" else round(104 / scale) if period == "W" else round(24 / scale) if period == "M" else 0
        case "3y":
            return round(756 / scale) if period == "D" else round(156 / scale) if period == "W" else round(36 / scale) if period == "M" else 0

    # OPTIONAL: make failures obvious instead of returning None
    raise ValueError(f"Unsupported realized_volatility_period='{realized_volatility_period}'")
