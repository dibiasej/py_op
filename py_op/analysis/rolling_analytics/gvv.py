...
from trash.market_price_processor import MarketPriceProcessor
from trash.volatility_processor import SkewProcessor
from calc_engine.vol_engine.interpolation_models import GVV

from data.price_data import process_price_data
from utils import date_utils

"""
We're going to get rid of this and move it into implied values
"""
...
def _gvv_time_series_helper(ticker, start, end, option_exp = '1M', moneyness = .3, steps = 10, otm_options = True):
    ...
    for i in range(len(common_dates)):
        ...
        if otm_options == True:
            ...
            strikes, ivs = zip(*SkewProcessor().get_ivs(spot, prices, strikes, dte))
            ...
            strikes, ivs = SkewProcessor().equal_put_call_skew(spot, call_data, put_data, dte)
