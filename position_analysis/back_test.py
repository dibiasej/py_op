...
def historical_option_price(ticker: str, K: int, start_date: str, end_date: str, expiration: str, option_style: str = "european", option_type: str = "call") -> list[float]:
    ...
    print(f"close dates: {close_dates}\n")

    for i, close_date in enumerate(close_dates):
        #print(f"close date: {close_date}, {i}\n")
        try:

            option_price: float = OptionFactory().create_option(ticker, K, expiration, close_date, option_style, option_type).get_price()
        
            #print(f"Option Factory: {OptionFactory().create_option(ticker, K, expiration, close_date, option_style, option_type)}\n")
            option_price_list.append(option_price)
            date_list.append(close_date)
        except:
            print(f"failed close date: {close_date}\n")
            continue

    print(f"date list after: {date_list}\n")

    print(f"op prices: {option_price_list}\n")

    return option_price_list, date_list
...
def position_back_test(ticker: str, start_date: str, end_date: str, position: Position):
    ...
    for pos in position:
        ...
        option_price, date = historical_option_price(ticker, strike, start_date, end_date, expiration, option_type=option_type)
...
def main():
    ...
    option_backtest_date_check("SPY", "2024-02-10", "2024-04-19", "2024-04-19")
