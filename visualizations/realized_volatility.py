from calc_engine.volatility.realized_volatility import historical_price_volatility as rv
...
class ImpliedVolatilityFactory:
    ...
    def create_implied_volatility_object(self, ticker: str, start_date: str, end_date: str, dtes: int, iv_method: str, frequency: str = "D", option_type='call') -> list[float]:
        ...
        close_prices = p_data.get_close_prices(ticker, start_date, end_date, frequency)
...
def main():
    ...
    start_date = str(st.date_input("Enter the Start Date ('YYYY-MM-DD')"))
    end_date = str(st.date_input("Enter the End Date ('YYYY-MM-DD')"))
    ...
    if vix_bool:
        vix_close_prices = p_data.get_close_prices("^VIX", start_date, end_date, freq = r_vol_freq) / 100
